...
public class OrganizationsController(IMediator mediator) : ControllerBase
{
    ...
    {
        var result = await mediator.Send(new GetOrganizations(page, pageSize), cancellationToken);
        return Ok(result);
    }
    ...
    {
        return await mediator.Send(new GetOrganization(id), cancellationToken);
    }
    ...
    {
        var dto2 = await mediator.Send(new CreateOrganization(dto.Name, dto.OrgNo, dto.Phone, dto.PhoneMobile, dto.Email, dto.Address), cancellationToken);
    ...
    {
        await mediator.Send(new DeleteOrganization(id), cancellationToken);
    }
}
