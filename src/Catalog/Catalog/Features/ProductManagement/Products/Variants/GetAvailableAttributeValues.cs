...
public record GetAvailableAttributeValues(string ProductIdOrHandle, string AttributeId, IDictionary<string, string?> SelectedAttributeValues) : IRequest<IEnumerable<AttributeValueDto>>
{
    public class Handler(ProductVariantsService productsService) : IRequestHandler<GetAvailableAttributeValues, IEnumerable<AttributeValueDto>>
    {
        ...
        {
            var values = await productsService.GetAvailableAttributeValues(request.ProductIdOrHandle, request.AttributeId, request.SelectedAttributeValues, cancellationToken);
