...
public static class Endpoints
{
    ...
    public static async Task<Results<Ok<IEnumerable<ProductOptionDto>>, BadRequest>> GetProductOptions(string organizationId, int id, string? variantId, IMediator mediator, CancellationToken cancellationToken)
    {
        return TypedResults.Ok(await mediator.Send(new GetProductOptions(organizationId, id, variantId), cancellationToken));
    }

    public static async Task<Results<Ok<OptionDto>, BadRequest>> CreateProductOption(string organizationId, int id, CreateProductOptionData data, IMediator mediator, CancellationToken cancellationToken)
    {
        return TypedResults.Ok(await mediator.Send(new CreateProductOption(organizationId, id, data), cancellationToken));
    }

    public static async Task<Results<Ok<OptionDto>, BadRequest>> UpdateProductOption(string organizationId, int id, string optionId, UpdateProductOptionData data, IMediator mediator, CancellationToken cancellationToken)
    {
        return TypedResults.Ok(await mediator.Send(new UpdateProductOption(organizationId, id, optionId, data), cancellationToken));
    }

    public static async Task<Results<Ok, BadRequest>> DeleteProductOption(string organizationId, int id, string optionId, IMediator mediator)
    {
        await mediator.Send(new DeleteProductOption(organizationId, id, optionId));
        return TypedResults.Ok();
    }

    public static async Task<Results<Ok<OptionValueDto>, BadRequest>> CreateProductOptionValue(string organizationId, int id, string optionId, CreateProductOptionValueData data, IMediator mediator, CancellationToken cancellationToken)
    {
    ...
    public static async Task<Results<Ok, BadRequest>> DeleteProductOptionValue(string organizationId, int id, string optionId, string valueId, IMediator mediator, CancellationToken cancellationToken)
    {
        await mediator.Send(new DeleteProductOptionValue(organizationId, id, optionId, valueId), cancellationToken);
        return TypedResults.Ok();
    }

    public static async Task<Results<Ok<IEnumerable<OptionValueDto>>, BadRequest>> GetProductOptionValues(string organizationId, int id, string optionId, IMediator mediator)
    {
        return TypedResults.Ok(await mediator.Send(new GetOptionValues(organizationId, optionId)));
    }

    public static async Task<Results<Ok<IEnumerable<OptionGroupDto>>, BadRequest>> GetProductOptionGroups(string organizationId, int id, IMediator mediator)
    {
        return TypedResults.Ok(await mediator.Send(new GetProductOptionGroups(organizationId, id)));
    }

    public static async Task<Results<Ok<OptionGroupDto>, BadRequest>> CreateProductOptionGroup(string organizationId, int id, CreateProductOptionGroupData data, IMediator mediator)
    {
        return TypedResults.Ok(await mediator.Send(new CreateProductOptionGroup(organizationId, id, data)));
    }

    public static async Task<Results<Ok<OptionGroupDto>, BadRequest>> UpdateProductOptionGroup(string organizationId, int id, string optionGroupId, UpdateProductOptionGroupData data, IMediator mediator)
    {
        return TypedResults.Ok(await mediator.Send(new UpdateProductOptionGroup(organizationId, id, optionGroupId, data)));
    }

    public static async Task<Results<Ok, BadRequest>> DeleteProductOptionGroup(string organizationId, int id, string optionGroupId, IMediator mediator)
    {
