...
public class CampaignsController(IMediator mediator) : ControllerBase
{
    ...
    {
        return await mediator.Send(new GetCampaignsQuery(page, pageSize, searchString, sortBy, sortDirection), cancellationToken);
    }

    [HttpGet("{id}")]
    public async Task<CampaignDto?> GetCampaign(string id, CancellationToken cancellationToken)
    {
        return await mediator.Send(new GetCampaignQuery(id), cancellationToken);
    }

    [HttpPost]
    public async Task<CampaignDto> CreateCampaign(CreateCampaignDto dto, CancellationToken cancellationToken)
    {
        return await mediator.Send(new CreateCampaignCommand(dto.Name), cancellationToken);
    }

    [HttpPut("{id}")]
    public async Task UpdateCampaign(string id, UpdateCampaignDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new UpdateCampaignCommand(id, dto.Name), cancellationToken);
    }
