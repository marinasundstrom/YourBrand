...
public class SitesController(IMediator mediator) : ControllerBase
{
    ...
    {
        return await mediator.Send(new GetSitesQuery(page - 1, pageSize, searchString, sortBy, sortDirection), cancellationToken);
    }

    [HttpGet("{id}")]
    public async Task<SiteDto?> GetSite(string id, CancellationToken cancellationToken)
    {
        return await mediator.Send(new GetSiteQuery(id), cancellationToken);
    }

    [HttpPost]
    public async Task<SiteDto> CreateSite(CreateSiteDto dto, CancellationToken cancellationToken)
    {
        return await mediator.Send(new CreateSiteCommand(dto.Name, dto.CreateWarehouse), cancellationToken);
    }

    [HttpPut("{id}")]
    public async Task UpdateSite(string id, UpdateSiteDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new UpdateSiteCommand(id, dto.Name), cancellationToken);
    }
