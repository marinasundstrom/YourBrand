...
public class WarehousesController(IMediator mediator) : ControllerBase
{
    ...
    {
        return await mediator.Send(new GetWarehousesQuery(page - 1, pageSize, siteId, searchString, sortBy, sortDirection), cancellationToken);
    }

    [HttpGet("{id}")]
    public async Task<WarehouseDto?> GetWarehouse(string id, CancellationToken cancellationToken)
    {
        return await mediator.Send(new GetWarehouseQuery(id), cancellationToken);
    }

    [HttpPost]
    public async Task<WarehouseDto> CreateWarehouse(CreateWarehouseDto dto, CancellationToken cancellationToken)
    {
        return await mediator.Send(new CreateWarehouseCommand(dto.Name, dto.SiteId), cancellationToken);
    }

    [HttpPut("{id}")]
    public async Task UpdateWarehouse(string id, UpdateWarehouseDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new UpdateWarehouseCommand(id, dto.Name, dto.SiteId), cancellationToken);
    }
