...
public class WarehouseItemsController(IMediator mediator) : ControllerBase
{
    ...
    {
        return await mediator.Send(new GetWarehouseItems(page - 1, pageSize, warehouseId, itemId, searchString, sortBy, sortDirection), cancellationToken);
    }

    [HttpGet("{id}")]
    public async Task<WarehouseItemDto?> GetItem(string warehouseId, string id, CancellationToken cancellationToken)
    {
        return await mediator.Send(new GetWarehouseItem(warehouseId, id), cancellationToken);
    }
    ...
    {
        return await mediator.Send(new AddWarehouseItem(dto.ItemId, warehouseId, dto.Location, dto.QuantityOnHand, dto.QuantityThreshold), cancellationToken);
    }

    [HttpPut("{id}")]
    public async Task UpdateItem(string warehouseId, string id, UpdateWarehouseItemDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new UpdateWarehouseItem(warehouseId, id, dto.Location), cancellationToken);
    }
    ...
    {
        await mediator.Send(new AdjustQuantityOnHand(warehouseId, id, dto.Quantity), cancellationToken);
    }
    ...
    {
        await mediator.Send(new PickWarehouseItems(warehouseId, id, dto.Quantity, dto.FromReserved), cancellationToken);
    }

    [HttpPut("{id}/Ship")]
    public async Task ShipItems(string warehouseId, string id, ShipItemsDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new ShipWarehouseItems(warehouseId, id, dto.Quantity, dto.FromPicked), cancellationToken);
    }

    [HttpPut("{id}/Receive")]
    public async Task ReceiveItems(string warehouseId, string id, ReceiveItemsDto dto, CancellationToken cancellationToken)
    {
        await mediator.Send(new ReceiveWarehouseItems(warehouseId, id, dto.Quantity), cancellationToken);
    }

    [HttpDelete("{id}")]
    public async Task DeleteItem(string warehouseId, string id, CancellationToken cancellationToken)
    {
        await mediator.Send(new DeleteWarehouseItem(warehouseId, id), cancellationToken);
    }
}
