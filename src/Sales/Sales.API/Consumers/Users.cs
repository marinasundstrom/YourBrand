...
public class SalesUserCreatedConsumer : IConsumer<UserCreated>
{
    ...
    public async Task Consume(ConsumeContext<UserCreated> context)
    {
        try 
        {
            ...
            var result = await _mediator.Send(new Sales.Features.OrderManagement.Users.CreateUser($"{message2.FirstName} {message2.LastName}", message2.Email, message.TenantId, message2.UserId));
        }
        catch(Exception e) 
        {
